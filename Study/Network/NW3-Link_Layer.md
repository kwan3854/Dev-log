...
2. 비트들 뒤에 어떤 비트들을 더 붙인다. (4에서 붙인 비트들에대해 설명)

3. 그리고 이 비트들을 이어서 하나의 숫자로 본다.

4. 이 때 이 숫자를 R이라는 숫자로 나누었을 때 나누어 떨어지게 한다.

   즉, 비트들 뒤에 R이라는 숫자에 나누어 떨어지도록 비트를 붙였던 것이다.

5. R 값은 bit가 깨졌을때 잘 나누어 떨어지는 숫자로 잘 정해야 한다.
